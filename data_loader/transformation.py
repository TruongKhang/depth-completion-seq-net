...
class RandomCrop(object):
    ...
    def crop_or_pad(self, in_tensor, target_size):
        h, w = in_tensor.shape[1], in_tensor.shape[2]
        new_h, new_w = target_size[0], target_size[1]
        ...
        if new_w < w:
            ...
            left = (new_w - w) // 2
            right = new_w - w - left
        patch_in_tensor = F.pad(in_tensor, [left, right, top, bottom])
        return patch_in_tensor

    def __call__(self, sample):
        output = ()
        for i in range(len(sample)-1):
            elem = sample[i]
            patch = self.crop_or_pad(elem, self.output_size)
            output += (patch, )
        # check the last element is a tuple (score_map, mask) or not
        if type(sample[-1]) is tuple:
            gt_depth, mask = sample[-1]
            patch_gt_depth = self.crop_or_pad(gt_depth, self.output_size)
            patch_mask = self.crop_or_pad(mask, self.output_size)
            output += ((patch_gt_depth, patch_mask), )
        else:
            output += (self.crop_or_pad(sample[-1], self.output_size), )
